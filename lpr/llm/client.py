...
# 🔴 ค่าเริ่มต้นเปลี่ยนเป็น **รับเลขสั้น** 2026-09-16 (v0.5.0)
# Toy ส่งภาพป้ายจริงมาห้าใบ มีสองใบเป็นป้ายเลขสวย (`๗กข ๑๓๕` สามหลัก · `ถม ๖๗`
# สองหลัก) ซึ่งด่านสี่หลักตีตกทั้งคู่ · **หลักฐานหน้างานชนะกฎที่จำต่อกันมา**
# และตรงกับต้นฉบับกรมขนส่งที่บอกว่าเลขทะเบียนคือ 1 ถึง 9999
#
# ⚠️ ราคาที่จ่ายไปจริงๆ: ด่านนี้เคยเป็นตัวเดียวที่จับ "โมเดลอ่านเลขขาดไปหนึ่งตัว"
# ได้ · ตั้งแต่นี้ `3869` ที่อ่านได้แค่ `386` จะผ่านออกไปเป็นทะเบียนที่ถูกรูป
# **ความแม่นจึงเหลือทางเดียวคือ `/truth` + `/v1/accuracy`** ไม่มีทางลัดอีกแล้ว
# ปิดกลับด้วย LPR_ALLOW_SHORT_DIGITS=false ถ้าไซต์ไหนมีแต่ป้ายสี่หลัก
...
def _all_zero(digits: str) -> bool:
    """เลขทะเบียนที่เป็นศูนย์ล้วน = ไม่ใช่ทะเบียนที่มีอยู่จริง

    🔴 ด่านนี้เกิดจากบั๊กจริง 2026-09-16 · prompt v2-v4 ใช้ทะเบียนจริงของรถทดสอบ
    (`6กว 3869`) เป็นตัวอย่างในคำสั่ง แล้ววันที่โมเดลอ่านภาพไม่ออก (ป้ายเลขไทย)
    **มันลอกตัวอย่างนั้นมาตอบ พร้อมความมั่นใจ 0.9** และคำตอบนั้นเข้ารูปทะเบียนครบ
    ทุกด่าน ปลายทางแยกไม่ออกเลยว่าเป็นทะเบียนของรถคันอื่นที่อยู่ในคำสั่งของเราเอง
    วัดจริง: ภาพ `๑กข๑๒๓๔` และ `๗กข๑๓๕` ได้ `6กว 3869` กลับมาทั้งคู่

    v5 เปลี่ยนตัวอย่างทั้งหมดเป็นเลขศูนย์ล้วน ซึ่งกรมขนส่งไม่เคยออก (เริ่มที่ 1)
    **และด่านนี้คือครึ่งหลังของการแก้**: ลอกมาเมื่อไหร่เราจับได้ 100% เพราะมัน
    เป็นทะเบียนที่มีอยู่จริงไม่ได้ · แก้แต่ prompt อย่างเดียวคือฝากไว้กับคำสั่ง
    ซึ่งเป็นสิ่งที่พังมาแล้วทั้งวันนี้

    ⚠️ รูป `commercial` มีศูนย์นำหน้าได้จริง (`10-0001` ต้นฉบับบอก 0001 ถึง 9999)
    ที่กันคือ **ศูนย์ล้วน** ไม่ใช่ศูนย์นำหน้า
    """
...
def _plate_problem(t: str) -> str:
    ...
    if _all_zero(digits):
        return ("เลขท้ายเป็นศูนย์ล้วน ซึ่งไม่มีอยู่จริง"
                " · น่าจะลอกตัวอย่างในคำสั่งมาตอบ แปลว่ามันอ่านภาพนี้ไม่ออก")
    if len(digits) != 4:
...
def _split_plate(raw: Any) -> Tuple[str, str, str, str, str]:
    ...
    for name, rx in _PLATE_PATTERNS:
        ...
        if m:
            ...
            if _all_zero(digits):
                # ตัวอย่างจากคำสั่งของเราเอง ไม่ใช่ป้ายในภาพ · ดู _all_zero
                return "", "", "", "", "unknown"
