...
# 🔴 อยู่ในรถเข็นหรือเดินเอง · Toy สั่ง 2026-09-14 ("กรณีเป็นเด็ก อยากให้แยกว่า
# เป็นเด็กในรถเข็นไหม")
#
# ทำไมเป็นช่องของ **ทุกคน** ไม่ใช่ช่องที่โผล่มาเฉพาะตอน age_group = child
#   1. ฟิลด์ที่มีบ้างไม่มีบ้างตามค่าช่องอื่น คือฟิลด์ที่ปลายทาง parse พังในวันที่
#      เจอเคสที่สอง · หลักเดียวกับ `model` และ `nationality` ที่คีย์อยู่เสมอ
#   2. **คนที่นั่งรถเข็นไม่ได้มีแต่เด็ก** ผู้สูงอายุนั่งวีลแชร์เข้าอาคารทุกวัน
#      และนั่นคือข้อมูลที่ รปภ. หน้าประตูอยากรู้พอๆ กัน · ถามครั้งเดียวได้สองเรื่อง
#      ไม่เสีย token เพิ่ม
#   3. อายุที่โมเดลอ่านจากภาพ CCTV พลาดได้ ถ้าผูกช่องนี้ไว้กับ age_group
#      เด็กที่ถูกอ่านเป็น 13-19 จะทำให้ "เด็กในรถเข็น" หายไปทั้งแถว
#
# ⚠️ `walking` = เดินหรือยืนด้วยขาตัวเอง (ถือไม้เท้า/ช่วยพยุงก็ยังนับ)
# ⚠️ **คนเข็นไม่ใช่คนถูกเข็น** แม่ที่เข็นรถเข็นคือ `walking` ส่วนเด็กในรถเข็น
# เป็นอีกคนหนึ่งที่มี mobility = stroller · สองคน ไม่ใช่คนเดียว
# ⚠️ มองไม่เห็นช่วงล่าง = `unknown` ห้ามเดาว่า walking
# ค่านี้เห็นจากภาพล้วนๆ (มีล้ออยู่ใต้ตัวเขาไหม) ไม่ใช่การตีความเหมือนอารมณ์
Mobility = Literal["walking", "stroller", "wheelchair", "carried", "other",
                   "unknown"]
...
class PersonOut(BaseModel):
    ...
    **อ่านคำเตือนยาวที่ `Nationality` ให้จบก่อนเปิด** โดยย่อ: ภาพบอกสัญชาติไม่ได้
    ...
    🔴 **คนเข็นเป็น `walking` คนที่นั่งอยู่ในรถเข็นเป็นอีกคนที่เป็น `stroller`**
    ปลายทางที่อยากได้ "เด็กในรถเข็นกี่คน" ดูที่ `summary.child_in_stroller`
    หรือกรอง `age_group == "เด็ก"` คู่กับช่องนี้เอาเอง

    มองไม่เห็นช่วงล่างของตัว = `unknown` ไม่ใช่ `walking` · ดู schemas.Mobility"""
    mobility_confidence: float = Field(0.0, ge=0.0, le=1.0)
